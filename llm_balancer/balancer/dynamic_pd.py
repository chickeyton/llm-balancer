...
class DynamicPd:
    ...
    def _gather_state(self):
        ...
        ttft_quantile = np.quantile(self._ttft_history, self._balancer.config.service_level_obj.quantile_p)
        tpot_quantile = np.quantile(self._tpot_history, self._balancer.config.service_level_obj.quantile_p)
