...
class LMCacheKvConnector(KvConnector):
    ...
    def __init__(self, lmcache_port: int, is_p2p_enabled: bool):
        self._kv_manager = controller_manager.LMCacheControllerManager(
            {"pull": f"0.0.0.0:{lmcache_port}", "reply": None}
        )
                ...
                if last_end == -1:
                    ...
                    if cache_info is not None:
                        if last_end == cache_info[1]:
                            ...
                            layout_info[matched_instance] = (matched_location, end)
            last_end = end
        return dict(zip(layout_info.keys(), [v[1] for v in layout_info.values()]))
